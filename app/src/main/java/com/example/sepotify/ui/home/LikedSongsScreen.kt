...
fun LikedSongsScreen(
    ...
    viewModel: LikedSongsViewModel = koinViewModel(parameters = { parametersOf(userId) })
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.liked_songs_title)) },
                navigationIcon = {
        ...
        Box(modifier = Modifier.padding(padding)) {
            LazyColumn(
                ...
                itemsIndexed(
                    ...
                    SwipeToDismissSongItem(
                        song = song,
                        onRemove = { viewModel.unlikeSong(song.id) },
                        onClick = {
            ...
            if (pagingItems.itemCount == 0 && pagingItems.loadState.refresh !is LoadState.Loading) {
                EmptyState(
                    ...
                    title = stringResource(R.string.no_liked_songs),
                    subtitle = stringResource(R.string.no_liked_songs_subtitle)
                )
            }
        }
    }
